...
c      write(out_unit,*) 'murrell',Qsym,pot0
...
c---------------------------------------------------------------------
      ...
      write(out_unit,*) 'nat,nat_act',mole%nat,mole%nat_act
      write(out_unit,*) 'nb_act1',mole%nb_act1
      write(out_unit,*) 'nb_inact2n',mole%nb_inact2n
      END IF
c---------------------------------------------------------------------
...
c        -------------------------------------------------------------
         write(out_unit,*) ' WARNING :  the ONLY active variable '
c        write(out_unit,*) '           is an angle'
c        i_type_act =  3
         write(out_unit,*) '           is cos(angle)'
         i_type_act = -3
c        -------------------------------------------------------------
...
c      write(out_unit,*) 'test d0h',deriv
...
C================================================================
     ...
     *                        inTOout)
      ...
      IMPLICIT NONE
       ...
       integer :: k_act,i_act,j_act,k,kl,vi
       integer :: i_sym_act,i_type_act,i_qsym_inact
...
c----- for debuging ----------------------------------
      character (len=*), parameter :: name_sub='calc_dnQgene'
      logical, parameter :: debug=.FALSE.
c     logical, parameter :: debug=.TRUE.
c----- for debuging ----------------------------------
...
c---------------------------------------------------------------------
      IF (debug) THEN
        write(out_unit,*) 'BEGINNING ',name_sub
        write(out_unit,*) 'nb_Qgene',nb_Qgene
        write(out_unit,*) 'iq_gene',iq_gene
      END IF
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
       ...
       CALL sub_ZERO_TO_dnS(dnQgene,nderiv)


      IF (inTOout) THEN
       IF (iq_gene == 1) THEN ! angle
         dnQgene%d0          = Qgene(iq_gene)
         IF (nderiv >= 1) dnQgene%d1(iq_gene) = ONE
       ELSE IF (iq_gene == 2 .OR. iq_gene == 3) THEN
         dnQgene%d0          = Qgene(iq_gene)
         IF (nderiv >= 1) dnQgene%d1(iq_gene) = ONE
         ...
         DO kl=1,nn(iq_gene)
           ...
           dnQgene%d0 = dnQgene%d0 + F(kl,iq_gene) * dc0

           IF (nderiv >= 1) 
     *       dnQgene%d1(1) = dnQgene%d1(1) + F(kl,iq_gene) * dc1

           IF (nderiv >= 2) 
     *       dnQgene%d2(1,1) = dnQgene%d2(1,1) + F(kl,iq_gene)*dc2

           IF (nderiv >= 3) 
     *       dnQgene%d3(1,1,1)=dnQgene%d3(1,1,1)+F(kl,iq_gene)*dc3
...
c---------------------------------------------------------------------
      IF (debug) THEN
        write(out_unit,*) 'dnQgene : ',Qgene(:)
        CALL write_dnS(dnQgene,nderiv)
        write(out_unit,*) 'END ',name_sub
      END IF
c---------------------------------------------------------------------
...
c        -------------------------------------------------------------
         IF (mole%nb_inact .GT. nb_inactb) THEN
           ...
           write(out_unit,*) 'nb_inact(',mole%nb_inact,')>nb_inactb(',
     *                 nb_inactb,')'
           STOP
         END IF
